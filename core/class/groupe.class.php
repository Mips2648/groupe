...
class groupe extends eqLogic {
    ...
    public static function pull($_option) {
		...
		if (is_object($groupe) && $groupe->getIsEnable() == 1) {
				$cache = cache::byKey('groupe::' . $groupe->getId() . '::' .$_option['event_id'], false, true);
				if($cache->getValue() != $_option['value']){
					cache::set('groupe::' . $groupe->getId() . '::' . $_option['event_id'],$_option['value'], 0);
					$groupe->execute($_option['groupe_id']);
				}
				else{
					log::add('groupe', 'debug', 'Pas de changement');
				}
        }
    }
	...
	public function getState($i,$j,$etat) {
